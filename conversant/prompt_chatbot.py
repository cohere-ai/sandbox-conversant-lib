...
class PromptChatbot(Chatbot):
    ...
    def check_and_adjust_prompt_size(self, prompt: str, formatted_query: str) -> Tuple[dict,str]:
        """Check if the prompt size is smaller than the MAX_PROMPT_SIZE.
            
        If not, try to adjust max_context_lines until a possible the prompt size.
            
        If is not possible, just send an error message.

        Args:
            prompt (str): Promp that will be send to co.generate
            formatted_query (str): Last message from user in the correct format

        Returns:
            Tuple[dict,str]: A dict with the status of the response and output message and a new prompt if changed 
        """    
        ...
        while (curr_size > self.max_prompt_size) and (self.chatbot_config['max_context_lines'] > 1):
            ...
            response["output_message"] = f"The max_context_lines was reduced from {original_max_context_lines} to {self.chatbot_config['max_context_lines']} so that the total amount of tokens does not exceed 2048."
        ...
        if curr_size > self.max_prompt_size:
            ...
            response["output_message"] = f"Total number of tokens (prompt and prediction) cannot exceed 2048. Try using a shorter start prompt, smaller text messages or a smaller max_tokens value. Details: \n - Start Prompt: {self.start_prompt_size} tokens \n - Messages in chat: {original_size - self.start_prompt_size} tokens \n - Max tokens generated: {self.client_config['max_tokens']} tokens"
    ...
    def reply(self, query: str) -> Dict:
        ...
        Returns:
            Dict: Response containing the status, the reply generated by .generate and output message if has
        """    
    ...
    def check_prompt_size(self) -> None:
        ...
        if self.start_prompt_size > self.max_prompt_size:
            raise ValueError(f"Start Prompt Config has too many tokens: total number of tokens cannot exceed 2048 - received {self.start_prompt_size}. Try using a smaller description or texts in the example turns")
    ...
    def _validate_persona_dict(persona: Dict[str, Any], persona_path: str) -> None:
        ...
        if persona['client_config']['max_tokens'] >= MAX_PROMPT_SIZE:
            raise ValueError("too many tokens: total number of max_tokens cannot exceed 2048. Try using a smaller max_tokens value.")
